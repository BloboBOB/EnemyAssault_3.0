...
MAD_spawnciv =
{
	...
	if (count _temphouses > 0) then
	{
		for "_b" from 1 to _count do
		{
			...
			//nul = [_civ,"ran",10,10,west,1,0.4] execVM "scripts\ws_assassins.sqf";
	...
	[] spawn
	{
		while {true} do
		{
			...
			if (count _houses > 0) then
			{
				if (!_var) then
				{
					player setVariable ["MAD_housesNear", true, true];
				};
			}
			else
			{
				if (_var) then
				{
					player setVariable ["MAD_housesNear", false, true];
				};
			};
...
MAD_deleteCivs =
{
...

	{
		...
		_c = 0;
		{
			if (_civ distance _x > MAD_maxCivDistance and ((lineintersects [eyepos _x,getposasl _civ,_x,_civ]) || (terrainintersectasl [eyepos _x,getposasl _civ]))) then
			{
				_c = _c + 1;
			};
		} forEach _players;
